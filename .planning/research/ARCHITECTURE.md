...
│  └─────────────┘    └─────────────┘    └─────────────┘    └──────────────┘ │
├─────────────────────────────────────────────────────────────────────────┤
│              I/O e ciclo de vida de ficheiros (Paths + diálogos)         │
│  app.paths.cache / data  —  ficheiros temporários e metadados de job    │
│  URIs/caminhos devolvidos por diálogos  —  leitura cópia para cache      │
│  SaveFileDialog  —  destino escolhido pelo utilizador (export)          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| **Session / job state** | Lista de ficheiros selecionados, texto da marca, flags “a processar”, erros por ficheiro | Classe ou `dataclass` isolada da UI; sem lógica PDF |
...
| **Export UX** | Guardar resultado onde o utilizador escolher; mensagens claras | `SaveFileDialog` por ficheiro ou estratégia em lote (ZIP) — decisão de produto; erros com `ErrorDialog` |
...
└── <app_name>/                 # nome Briefcase (ex. assinatura_pdf)
    ...
    ├── session.py              # estado da sessão (ficheiros, texto, jobs)
...
- **`ui/` vs `pipeline/`:** separa widgets Toga (thread/UI) de código CPU e de ficheiros pesados, facilitando testes unitários do pipeline sem emulador.
...
**When to use:** Sempre que houver diálogo ou sequência que depende do utilizador (recomendado pela documentação Toga).  
...
### Pattern 3: Ficheiros temporários sob `app.paths.cache`
...
**Trade-offs:** Disco e gestão de limpeza; evita depender do CWD (anti-pattern documentado na [gestão de ficheiros Toga](https://github.com/beeware/toga/blob/main/docs/en/topics/file-management.md)).

## Data Flow

### Fluxo principal (utilizador)

```
[Utilizador: Adicionar ficheiros]
    ↓
...
Session armazena pathlib.Path / referências devolvidas pelo diálogo
    ↓
[Utilizador: edita texto marca + Ok]
    ↓
Para cada entrada: cópia opcional → cache → pipeline → PDF endurecido em cache
    ↓
[Export]
    ↓
SaveFileDialog (por ficheiro ou lote) → escrita no destino escolhido
    ↓
...
1. **Seleção multi-ficheiro:** resultado de `OpenFileDialog` → normalização para paths internos → exibição na lista.
2. **Processamento em lote:** fila explícita ou loop sequencial com cancelamento opcional; cada item produz artefacto em `cache/job_id/`.
...
|-------|---------------------------|
| Utilizador único, lotes pequenos (requisito) | Pipeline sequencial simples; limite explícito de N ficheiros ou MB totais na validação. |
| PDFs grandes / muitas páginas | Processar página a página ou documento a documento; libertar buffers; considerar limite de memória e mensagem “ficheiro demasiado grande”. |
...
**Why it's wrong:** Android pode sugerir ANR ou utilizador assume crash.  
**Do this instead:** Progresso por ficheiro (label ou determinate progress quando disponível) e erros incrementais.
...
| Play Store / assinatura | Fora do runtime do app | `briefcase package` exige assinatura para artefactos utilizáveis ([documentação Briefcase](https://briefcase.readthedocs.io/en/stable/reference/platforms/android/gradle.html)). |
...
2. **Modelo de sessão + lista de ficheiros** — estado em memória; UI para mostrar/remover itens; sem PDF ainda. *Desbloqueia:* contrato de dados para o resto do app.
...
6. **Export / download UX** — `SaveFileDialog`, nomes de ficheiro sugeridos, tratamento de cancelamento; limpeza de temporários; fluxo **Sair**. *Desbloqueia:* entrega utilizável para partilha com terceiros.

**Rationale de dependências:** diálogos e paths antes do pipeline pesado; pipeline “estreito” antes das regras visuais/PDF complexas; export por último para polarizar UX em cima de ficheiros já corretos.
